			...
			if [[ $dnload_repo_Run == true ]]; then
				...
				Task "* Download script" "
					...
					if [[ ! -s "${script_nm_Run}" ]]; then
						...
						cat "${script_nm_Run}" 2>/dev/null || Txt "none法顯示檔案內容"
						return 1
					fi
					...
					chmod +x "${script_nm_Run}" || {
						...
						Err "none法設定 ${script_nm_Run} 的執行權限"
						ls -la "${script_nm_Run}"
						return 1
					}
				"
...
function SysClean() {
	...
	*apt)
		...
		apt autoclean -y || {
			Err "自動Cleaning APT cache failed"
			return 1
		}
		;;
	*opkg)
