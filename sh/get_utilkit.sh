...
declare e loc m pkg_mgr="unknown"
...
for m in apt-get apk dnf opkg pacman yum zypper; do
	command -v "$m" &>/dev/null && pkg_mgr="$m" && break
done
...
function download_utilkit_sh() {
	if wget -qO "${HOME}/utilkit.sh" "https://raw.githubusercontent.com/OG-Open-Source/UtilKit/main/sh/localized/${lang}/utilkit.sh" 2>/dev/null; then
		printf "%b\n" "\x1b[0;32mDownloaded pre-localized version for ${lang}\x1b[0m"
	else
		printf "%b\n" "\x1b[0;33mPre-localized version not available, downloading default version...\x1b[0m"
		if ! wget -qO "${HOME}/utilkit.sh" "https://raw.githubusercontent.com/OG-Open-Source/UtilKit/main/sh/utilkit.sh"; then
			printf "%b\n" "\x1b[0;31mFailed to download UtilKit.sh\x1b[0m"
			exit 1
		fi
	fi
}
...
if [[ -f ${HOME}/utilkit.sh ]]; then
	...
	if ! grep -q "source ~/utilkit.sh" ~/.bashrc; then
		cat <<<"source ~/utilkit.sh" >>~/.bashrc
		printf "%b\n" "\x1b[0;32mAdded source command to ~/.bashrc\x1b[0m"
	fi

	printf "%b\n" "\x1b[0;32mUtilKit.sh has been installed successfully\x1b[0m"
	printf "%b\n" "\x1b[0;32mPlease run 'source ~/utilkit.sh' to use it in the current session\x1b[0m"
fi
