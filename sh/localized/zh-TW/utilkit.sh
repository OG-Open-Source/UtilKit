...
function SysInfo() {
	...
	Txt "${CLR8}${SLINE}${CLR0}"
	Txt "- 運行時間：\t\t${CLR2}$(uptime -p | sed -E 's/^up //; s/ year[s]?/ 年/g; s/ week[s]?/ 週/g; s/ day[s]?/ 天/g; s/ hour[s]?/ 小時/g; s/ minute[s]?/ 分鐘/g; s/, /，/g')${CLR0}"
	Txt "- 啟動時間：\t\t${CLR2}$(who -b | awk '{print $3, $4}' | sed -E 's/([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2})/\1 年 \2 月 \3 日 \4 點 \5 分/')${CLR0}"
	Txt "${CLR8}${SLINE}${CLR0}"
